# Версия: 0.8
# Статус: РАБОТАЕТ
# Реализация: Внешняя Функция превращения ФИО в логин (UserLogin) + feedback
# Проблемы:
...
$po, $ResultUL, $login, $u = $Null


# Подгрузка внешней функции UserLogin
# powershell -ExecutionPolicy bypass -File \\t2ru\folders\IT-Outsource\Scripts\PowerShell\Functions\Logging_v0.2.ps1
#. \\t2ru\folders\IT-Outsource\Scripts\PowerShell\Functions\UserLogin_v0.3.ps1
...
#"Дата ввода неправильного пароля  : " + $ResultUL.LastBadPasswordAttempt.DateTime
 "Ввод неправильного пароля" = $ResultUL.LastBadPasswordAttempt.DateTime
#"Дата установки пароля            : " + $ResultUL.PasswordLastSet.DateTime
 "Дата установки пароля" = $ResultUL.PasswordLastSet.DateTime
} 
$UserPropHash2
# $ResultUL.([datetime]::FromFileTime($ResultUL."msDS-UserPasswordExpiryTimeComputed"))
# Get-ADUser -Identity denis.tirskikh -Properties msDS-UserPasswordExpiryTimeComputed | select-object @{Name="ExpirationDate";Expression= {[datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed") }}
# Get-ADUser -Identity $Login -Properties msDS-UserPasswordExpiryTimeComputed.ExpirationDate
# "Срок действия пароля" = 
Get-ADUser -Identity $Login -Properties msDS-UserPasswordExpiryTimeComputed | select-object @{Name="Срок действия пароля до     ";Expression= {[datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed") }}
# "Срок действия пароля" = Get-ADUser -Identity $Login -Properties msDS-UserPasswordExpiryTimeComputed | select-object @{Name="Срок действия пароля";Expression= {[datetime]::FromFileTime($_."msDS-UserPasswordExpiryTimeComputed") }}
...
''
 ...
 if ($lo -eq 1) {
    ...
    Set-Clipboard -Value $ResultUL.SamAccountName
...
Write-host 'Информация о компьютерах пользователя из SCCM / GLPI / Spareparts:' -ForeGroundColor Yellow
...
<# Раздел определения диреткора отключаю. Но он ПОЛНОСТЬЮ РАБОЧИЙ
...
Конец раздела определения диреткора #> 
