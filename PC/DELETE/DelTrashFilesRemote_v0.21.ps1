...
.EXAMPLE
  ...
  \\t2ru\folders\IT-Outsource\Scripts\PowerShell\PC\DelTrashFilesRemoteV0.21.ps1
.INPUTS
  PC or IP
.OUTPUTS
  text to clipboard for bpm comments
.NOTES
  for all questions: denis.tirsikh@tele2.ru
  Future plans:
#>
...
[Environment]::NewLine 
  ...
  if ($OldTicket -eq $Null) {
   ...
   do {
    $T = $null
    $T = Read-Host "Укажи номер тикета, по которому ведёшь работу"
    $Ticket = $T
...
# Function DelTrash
. \\t2ru\folders\IT-Outsource\Scripts\PowerShell\SQL\WsirIt01_DelTrash_v0.2.ps1
...
DelTrash -Operation Write -SRnumber $SRnumber -SRlink $SRlink -AdminLogin $AdminLogin -UserLogin $UserProp.SamAccountName -PCname $PC
...
# $ForDelFold = @($wcdump1, $wcdump2, $wcUpdate1, $wctemp1, $wctemp2)
  $ForDelFold = $wcdump1, $wcdump2, $wcUpdate1, $wcUpdate3, $wctemp1, $wctemp2, $wctelemetry1, $Bartender
