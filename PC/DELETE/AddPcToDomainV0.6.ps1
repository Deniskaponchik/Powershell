# \\t2ru\folders\IT-Support\Scripts\ADComputers\AddPcToDomainV0.5.ps1
...
if ($Error[0] -like '*Не удается привязать аргумент*' -or $x -eq 0) {exit}   # Отменяем скрипт, если нажать Отмена или 5 попыток
...
Puse} #>
else {"Текущая ОС на компьютере                      :{0}" -f $OS.Caption  + $OS.CSDVersion
...
{ $DomainName = $OS.csname
      ...
      # Проверяем, есть ли учётка в домене
      # [string]$Checkip = [System.Net.Dns]::GetHostEntry($DomainName).addresslist[0].ipaddresstostring
      # if ($Checkip) {Write-Host  'Учётка ПК есть в домене. Предпочтительнее сменить имя' -ForegroundColor Red}

      try {[string]$Checkip = [System.Net.Dns]::GetHostEntry($DomainName).addresslist[0].ipaddresstostring}
      catch {Write-Host  'Машины с таким именем в доменен нет.' -ForegroundColor Green}
      if ($Checkip) {Write-Host  'Учётка ПК есть в домене. Предпочтительнее сменить имя' -ForegroundColor Red}
      

  [Environment]::NewLine
  $SmallCityName = Read-Host "Укажи новое имя машины, если желаешь его сменить"
  if ($SmallCityName) {$DomainName = $SmallCityName}
      "В домен будет добавлен компьютер  : {0}" -f $DomainName
      [Environment]::NewLine
      Write-Host "Проверка и удаление старой учётки компьютера из домена, если такая имелась" -ForegroundColor Red `n
      Remove-ADComputer -Identity $DomainName -confirm -ErrorVariable ErrDel
    } 
    else {"Текущее имя не соответствует стандарту Теле2   :{0}" -f $OS.csname
  do { $DomainNameNew = Read-Host "Новое доменное имя (максимум 15 символов)      " }
  while ($DomainNameNew.Length -ge 16)  # }while ($Error[0] -like '*Не удается*')
  Write-Host "Проверка и удаление старой учётки компьютера из домена, если такая имелась" -ForegroundColor Red `n
  # Remove-ADComputer -Identity $DomainNameNew
  try {Remove-ADComputer -Identity $DomainNameNew}
 #catch {Write-Host  "Машины с таким именем в доменен нет." -ForegroundColor Green}
  catch {Write-Host  "Машины с именем $DomainNameNew в доменен нет." -ForegroundColor Green}
}
''
...
<# Это рабочее:
if ($DomainCred -eq $False) {
Write-Host "Запрос прав администратора домена..." -ForegroundColor Red
$DomainCred = Get-Credential "t2ru\admin.ws."    
if ($Error[0] -like '*Не удается привязать аргумент*') {exit}}   # Остановить скрипт, если нажать Отмена       

if ($DomainNameNew -ne $null) {$DomainName = $DomainNameNew          # Финалочка с изменением имени
Add-Computer -DomainName 'corp.tele2.ru' -OUPath $ldap_path -ComputerName $ip -LocalCredential $LocalCred -NewName $DomainName -Restart `
-Force -Credential $DomainCred}
else{    # Финалочка без изменения имени:
Add-Computer -DomainName 'corp.tele2.ru' -OUPath $ldap_path -ComputerName $ip -LocalCredential $LocalCred -Restart -Credential $DomainCred -Force}
#>
$Error[0] = $null
do {  # $DomainCred -eq $Null
    ...
    if ($Null -eq $AdmWSDomCred -or $ErrAdd) { 
      ...
      if ($Error[0] -like '*Не удается привязать аргумент к параметру*') {exit}}
   
    #($Error[0]) = $null # Пережиток прошлого, когда возврат был на основании $Error[0]
   if ($null -ne $DomainNameNew) {   # с изменением имени  
    New-AdComputer -Name $DomainNameNew -SamAccountName $DomainName -Credential $AdmWSDomCred -Path $ldap_path  # Избавляет от некоторых ошибок
   ...
   else{  # БЕЗ изменения имени:
    New-AdComputer -Name $DomainName -SamAccountName $DomainName -Credential $AdmWSDomCred -Path $ldap_path   # Избавляет от некоторых ошибок
...
# Не удалось присоединить компьютер
  ...
  "`n"
    ...
    Write-Host ""
    Write-Host "Уд. рабочему столу не удалось подключиться к уд. компьютеру $ip по одной из след. причин:" -ForegroundColor Red
    ...
    Write-Host "Подсказка. Запустите скрипт:" -ForegroundColor Red
    Write-Host "\\t2ru\folders\IT-Outsource\Scripts,поправки\PowerShell\ForAddToDomain.ps1" -ForegroundColor Red  
}