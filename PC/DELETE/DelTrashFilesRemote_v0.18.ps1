...
Write-Host "1. Диск РАЗБИТ и его объём можно увеличить слиянием разделов. Добавить SSD или HDD. " -ForegroundColor Cyan
...
Write-Host "2. Диск РАЗБИТ и его объём можно увеличить слиянием разделов. Добавить SSD или HDD. (Вариант для УСТАРЕВШИХ Windows)" -ForegroundColor Cyan
...
$Oth2 = "Необходимо добавить $ActualSsdSize", "Затем установить на него $ActualWindowsVersion", 'Затем сделать слияние разделов на старом диске (который сейчас установлен на машине пользователя).', 'Провести работу с пользователем по обучению сохранению личных файлов на новый созданный диск D:'

Write-Host "3. На ПК НЕ УСТАНОВЛЕН SSD и объём памяти компьютера можно увеличить установкой SSD." -ForegroundColor Magenta
...
$Oth3 = "Необходимо добавить $ActualSsdSize", 'Затем клонировать на него текущую Windows или установить новую.', 'Потом оказать помощь пользователю по переносу личных файлов на диск D, очистив тем самым диск С: не менее чем на 150 ГБ.'

Write-Host "4. На ПК НЕ УСТАНОВЛЕН SSD и объём памяти компьютера можно увеличить установкой SSD. (Вариант для УСТАРЕВШИХ версий Windows)" -ForegroundColor Magenta
...
Write-Host "7. На ноутбуке всего один диск на 240 ГБ. Замена или добавление SSD на 500 ГБ." -ForegroundColor Yellow
$Oth7 = "Если устройство позволяет, то добавить $ActualSsdSize", 'Очистить потом диск C: не менее чем на 100 ГБ переносом пользовательской информации на новый диск', 'Если места для установки второго диска нет, то просто заменить на SSD 500 ГБ.', ''

Write-Host "8. На ноутбуке всего один диск на 240 ГБ. Замена или добавление SSD на 500 ГБ. (Вариант для УСТАРЕВШИХ Windows)" -ForegroundColor Yellow
...
Write-Host "9. На ПК не установлен HDD и объём памяти компьютера можно увеличить установкой HDD." -ForegroundColor Cyan
...
Write-Host "12. Полная замена устаревшего системного блока (Ниже i3 4-ой серии)." -ForegroundColor Yellow
$Oth12 = "Необходимо полностью заменить устаревший системный блок:", $Proc, '',  '!!! Если на старой машине имеется ОЗУ 8 ГБ ОДНОЙ ЦЕЛЬНОЙ планкой, то её снять и передать логисту на склад !!!', 'SSD-диск со старого ПК снять и передать логисту на склад', 'Старый монитор пользователя можно оставить ему вторым', '', 'Имя машины:', $PC, ''

Write-Host "13. Полная замена устаревшего системного блока (Ниже i3 4-ой серии). (Вариант для УСТАРЕВШЕЙ Windows)" -ForegroundColor Yellow
...
Write-Host "14. Полная замена устаревшего ноутбука (Ниже i3 4-ой серии)." -ForegroundColor Yellow
#$Oth10 = "Необходимо полностью заменить устаревший ноутбук. $Proc"
 ...
 Write-Host "15. Полная замена устаревшего ноутбука (Ниже i3 4-ой серии). (Вариант для УСТАРЕВШЕЙ Windows)" -ForegroundColor Yellow
#$Oth10 = "Необходимо полностью заменить устаревший ноутбук. $Proc"
...
#$Oth10 = "Необходимо полностью заменить устаревший ноутбук. $Proc"
 $Oth16 = "Заменить SSD на больший объём = 240 ГБ", 'В рамках тикетов System Monitoring Auto Create модернизации не проводим. Необходимо завести дополнительное обращение на Модернизацию рабочего места', 'Старый SSD-диск передать логисту на склад', "Уже далеко не первая заявка на очистку диска", '', "На основании тикета System Monitoring Auto Create: $Ticket", ''


 Write-Host "17. Заменить SSD на больший объём (Вариант для УСТАРЕВШЕЙ Windows)" -ForegroundColor Cyan
  $Oth17 = "Заменить SSD на больший объём = 240 ГБ", "Необходимо также ОБЯЗАТЕЛЬНО на новом диске обновить систему до $ActualWindowsVersion через Центр Программного обеспечения или установить её с нуля в случае недоступности таковой в Центре ПО.", 'Старый SSD-диск передать логисту на склад', '', "На основании заявки System Monitoring Auto Create: $Ticket", ''
...
elseif ($OSBuildNumber -like '19044') {Write-Host '21H1' -ForegroundColor Green}
