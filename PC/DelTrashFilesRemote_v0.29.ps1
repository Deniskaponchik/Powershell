...
# Write-Host 'Запись отладочной информации отключена' -ForegroundColor Magenta
    ...
    $ErrDisDebugInfo = 'Отключить ведение журнала записи отладочной информации при краше системы не удалось'
    $ErrDisDebugInfo
}
...
$FindIndexFile = Get-ChildItem $FindIndexPath -Force -recurse
...
$wsService = get-service -ComputerName $PC -Name WSearch
$wsService
$wsService.Stop()
#$wsService
#(get-service -ComputerName $PC -Name WSearch).Stop() -verbose
