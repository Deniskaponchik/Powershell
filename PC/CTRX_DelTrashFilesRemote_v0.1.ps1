...
$UserPropInfo = @($UserProp.displayname, $UserProp.title, $UserProp.mobilePhone, $UserProp.l, '', "Инструкция по очистке:", "https://wiki.tele2.ru/pages/viewpage.action?pageId=71622259")
#$x = 3
do {
 # $x = $x - 1
  ...
  foreach ($TicketFile in $TicketsFiles) {
    ...
    # if ($ChaNout -match "\b$ChassisType\b") {$TypePC = 'Ноутбук'}
      if ($TicketFile.Name -match $login) {
        Write-host 'По данному пользователю уже велась/ведётся работа в рамках (номер скопирован в буфер обмена):' -ForegroundColor Red
