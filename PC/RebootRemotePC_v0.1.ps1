...
if (!$ErrResPC) {
    Write-Host 'Команда отправки в перезагрузку прошла успешно.' -ForegroundColor Green
    'Отключение может занять не один десяток минут, если прилетали обновления...'
}
else {
    Write-Host 'Пришёл ответ, что команда отправки в ребут не сработала, но это не факт, поэтому включена отправка постоянных пингов, если вдруг компьютер ребутнётся...' -ForegroundColor Magenta
}

do {
    $PingOld = (Test-Connection -ComputerName $PC -Quiet)
} while ($PingOld -eq $True)
$wshell = New-Object -ComObject Wscript.Shell
$wshell.Popup("Устройство отключилось: $PC")

do {
    $PingOld = (Test-Connection -ComputerName $PC -Quiet)
} while ($PingOld -eq $False)
$wshell = New-Object -ComObject Wscript.Shell
