...
if (!$MAC_Address) {
    Write-Host 'Устройство - не компьютер ИЛИ недоступно. Будет предпринята попытка получения мака с DHCP-серверов.' -ForegroundColor Red
    <#
    $DHCP_Servers = @{
...
Write-Host "0. для заявки в bpm (xx-xx-xx-xx-xx-xx)" -ForegroundColor Green
