...
$MAC_Address = $Null
if (($m = Read-Host "Сетевое имя или IP-адрес") -ne "") {$PC = $m}
[Environment]::NewLine


if (Test-Connection -ComputerName $PC -Quiet) {
    # Проще всего получить мак с доступной в сети Windows-машины. Пробуем, проверяем:
...
if (!$MAC_Address) {    #Если устройство не Windows или не доступно
    ...
    foreach ($DHCP_Server in $DHCP_Servers) {        
        if (!$Split_Add) { 
            $DHCP_Server
            if ($PC -notlike '1*') { #Если ищем не по ip-адресу
...
# Заканчиваем скрипт, если мак так и не обнаружился:
if (!$MAC_Address) {
...
Write-Host "7. bpm: Резервирование IP адреса" -ForegroundColor Cyan
[string]$JoinAddBPM1 = $MAC_Address
Write-Host "8. bpm: Проверка MAC-адреса перед установкой ОС" -ForegroundColor Cyan
