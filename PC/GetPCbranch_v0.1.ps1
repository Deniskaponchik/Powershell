...
[array]$X = "Name`tDN`tType`tBranch`tCreated`tLastLogon`tOS"
...
$X += ($_.Name + "`t" + $_.DistinguishedName + "`t" + $dnarr[1].Substring(0,2).ToUpper() + "`t" + $dnarr[($dnarr.length-9)] + "`t" `
+ $_.Created.ToString("dd.MM.yyyy HH:mm:ss") + "`t" + $_.LastLogonDate.ToString("dd.MM.yyyy HH:mm:ss") + "`t" `
+ ($_.operatingSystem + " " + $_.operatingSystemServicePack).Trim()) + "`t" + $_.Description
}
