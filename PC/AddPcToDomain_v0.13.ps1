...
<# Можно включить логирование, если нужно. РАБОЧЕЕ
...
<# РАБОЧЕЕ. Убираю в связи с переход на подключение к БД WSIR-IT-01/ITsupport
    ...
    elseif ($Cut -like 'wsob' -or $Cut -like 'nbob') { $b = 'Orenburg'}
    ...
    elseif ($Cut -like 'wsvl' -or $Cut -like 'nbvl') { $b = 'Vladimir'}
    elseif ($Cut -like 'wsvk' -or $Cut -like 'nbvk') { $b = 'Vladivostok'}
    elseif ($Cut -like 'wsvd' -or $Cut -like 'nbvd') { $b = 'Volgograd'}
    elseif ($Cut -like 'wsva' -or $Cut -like 'nbva') { $b = 'Vologda'}
    elseif ($Cut -like 'wsvo' -or $Cut -like 'nbvo') { $b = 'Voronezh'}
    elseif ($Cut -like 'wsyr' -or $Cut -like 'nbyr') { $b = 'Yaroslavl'}
    elseif ($Cut -like 'wsyo' -or $Cut -like 'nbyo') { $b = 'Yoshkar-Ola'}
...
# Определение бранча в AD на основании имени машины:
[Environment]::NewLine
...
if ($Null -eq $b){
    $b = Read-Host "Бранч не найден. Введи вручную"
}
else {
