...
# Last Update:  Добавлено: несколько обработок ошибок, места для очистки из предложений Елисеева
...
#>
# Логирование в БД НОВОЕ через MS SQL Server
  $AdminLogin = $env:USERNAME
  $DateStart = Get-Date
  $ScriptName= $MyInvocation.MyCommand.Name
. \\t2ru\folders\IT-Outsource\Scripts\PowerShell\DataBase\SQL\WsirIt01_ScriptEx_v0.0.ps1
# -DateEnd '1/1/1753 12:00:00 AM' чтобы в базу занёсся NULL
  ScriptsExecute -Operation Insert -AdminLogin $AdminLogin -DateStart $DateStart -ScriptName $ScriptName -DateEnd '1/1/1753 12:00:00 AM' #$DateStart
...
#$StartScriptPath = "C:\Users\$env:USERNAME\Downloads\"
#$StartScriptPath = "C:\Users\denis.tirskikh\Downloads\"
$StartScriptPath = "D:\Logs\"
#$StartScriptPath

#$PC = "TEST"
$DateStart = Get-Date -Format "dd.MM.yyyy_HH.mm.ss"
#$DateStart
$StartScriptFileName = $PC+"_"+$DateStart+".log"
#$StartScriptFileName

$StartScriptTotal = $StartScriptPath + $StartScriptFileName
$StartScriptTotal

try {
  New-Item -Path "D:\" -Name "Logs" -ItemType "directory" #-ErrorAction Stop -ErrorVariable ErrCreateLogFolder
  New-Item -Path $StartScriptPath -Name $StartScriptFileName -ItemType "file" -ErrorAction Stop -ErrorVariable ErrCreateLogFile
  [Environment]::NewLine

  Start-Transcript -Path $StartScriptTotal -Append
  Write-Host  "Установлено логирование скрипта в файл:" -ForegroundColor Green
  Write-Host  $StartScriptTotal -ForegroundColor Green
}
catch {
  Write-Host  $ErrCreateLogFile -ForegroundColor Red
  Write-Host  "Логирование в файл не смогло запуститься. Проверь возможность существования пути:" -ForegroundColor Red
  Write-Host  $StartScriptTotal -ForegroundColor Red
}
[Environment]::NewLine
PAUSE
...
# Function DelTrash, которая взаимодействует с таблицей SmacPC в БД
...
[Environment]::NewLine
# Download from file function RegionCodes
. \\t2ru\folders\IT-Outsource\Scripts\PowerShell\DataBase\SQL\WSIRit01_RegionCodes_v0.1.ps1
# -Operation -Это название скрипта, для которого используем
...
if ($Null -eq $BPMgroup){
    Write-host "Не удалось определить ответственную группу ИТ для $Pcut" -ForegroundColor Red
    Write-host "Информация передана разработчикам скрипта для добавления в базу" -ForegroundColor Red
    $CommentForSmacPCTable = -join("Не удалось определить ответственную группу ИТ для: " , $Pcut)
}
else {
    ...
    [Environment]::NewLine
    try{
        #Set-Clipboard -Value $BPMgroup #-ErrorAction Stop -ErrorVariable ErrCopyBpmGroup
        $BPMgroup | Set-Clipboard
        Write-host 'Группа скопирована в буфер обмена' -ForegroundColor Green
        ...
        Write-Host "Не удалось скопировать группу bpm в буфер обмена" -ForegroundColor Red
        
        #$ErrCopyBpmGroupMy = "Не удалось скопировать группу bpm в буфер обмена"
        #Write-Host $ErrCopyBpmGroupMy -ForegroundColor Red
        #Write-Host $ErrCopyBpmGroup -ForegroundColor Red
        #$Error[0]
    }
}
#Pause
...
#$PC = Read-Host "Имя компьютера или IP-адрес"
 $PCinfoResult = PCinfo -PC $PC
#$PCinfoResult.UsersNamesArray
...
Write-host 'Далее будут удаляться профили пользователей, чей LastWriteTime более 60 дней' -ForegroundColor Red
#Write-Host 'Введи в поле нижe ФИО или логин пользователя, чей профиль не будет удаляться в рамках данной активности' -ForegroundColor Green
Write-Host 'Введи в поле нижe ЛОГИН пользователя, чей профиль не будет удаляться в рамках данной активности.' -ForegroundColor Green
Write-Host 'Скопировать можно из списка выше.' -ForegroundColor Green

<#
$UserInput = Read-Host "Login "
$UserLogin = $UserInput.trim()
$UserLogin
if ($PCinfoResult.UsersNamesArray -eq $UserLogin){'Бинго!'}
PAUSE #>
do {
  $UserInput = Read-Host "Login "
  $UserLogin = $UserInput.trim()
  $UserLogin
#} while ( #c while проверка наличия в массиве не заработала
 } until (
  #Спрашивать до тех пор, пока $UserInput не станет попадать в $PCinfoResult.UsersNamesArray
   $PCinfoResult.UsersNamesArray -eq $UserLogin
  #$PCinfoResult.UsersNamesArray -ne $UserLogin
)
...
#$UserFIO
#if ($Null -eq $UserFIO){  $UserFIO = ''}
#if ($UserLogin.Contains(".")){
if ($Null -ne $UserFIO){
    Write-Host 'Выбран доменный пользователь' -ForegroundColor Magenta
  # Загрузка функции UserLogin, которая по логину ищет информацию в AD
  ...
  #. \\t2ru\folders\IT-Outsource\Scripts\PowerShell\Users\UserLogin_v0.5.ps1
   . \\t2ru\folders\IT-Outsource\Scripts\PowerShell\Users\UserLogin_v0.6.ps1
  #[string]$login = UserLogin
   $UserProp = UserLogin -$UserLogin
  ...
  try {
      ...
      $UserPropInfo | Set-Clipboard
      Write-Host 'Контактные данные пользователя скопированы в буфер обмена :' -ForegroundColor Green
  }
  catch {
      Write-Host 'Контактные данные пользователя не удалось скопировать в буфер обмена' -ForegroundColor RED `n
  }
}else{
    Write-Host 'Выбран локальный пользователь' -ForegroundColor Magenta
}
[Environment]::NewLine
#PAUSE
...
### Фиксируем собранные данные таблицу [Itsupport].[dbo].[SmacPC]
 $AdminLogin = $env:USERNAME
 DelTrash -Operation Write -SRnumber $SRnumber -SRlink $SRlink -AdminLogin $AdminLogin -PCname $PC -WorkType $WorkType -IncidentType $IncidentType -Comment $CommentForSmacPCTable -UserLogin $UserLogin #-UserLogin $UserProp.SamAccountName
 PAUSE




#####   ДАЛЬШЕ ВСЁ ПРО ОЧИСТКУ   ######


### Если компьютер недоступен ###
#$PC = 'wsir-it-04'
...
Write-Host 'Попытка отключения гибернации:' -ForegroundColor Magenta
# $PC = 'wsir-it-05'
# Write-Host "Запрос прав администратора домена (admin.ws.)" -ForegroundColor Red
# $AdmWSDomCred = Get-Credential
...
try {
    ...
    Invoke-WMIMethod -path Win32_Process -name Create -argumentlist "powercfg.exe -h off" -ComputerName $PC -ErrorVariable ErrDelHyb -Verbose -ErrorAction Stop
    Write-Host 'Гибернация отключена' -ForegroundColor Green
    # Wait-Event -Timeout 30
    # $ErrDelHyb
}
catch {
    $ErrDelHybMy = 'Отключить гибернацию не удалось'
    Write-Host $ErrDelHybMy -ForegroundColor Red
    Write-Host $ErrDelHyb -ForegroundColor Red
    $Error[0]
    $ErrDelHybArray = $ErrDelHybMy, $ErrDelHyb, $Error[0]
    #$ErrDelHybArray
}
[Environment]::NewLine
...
Write-Host 'Попытка отключения ведение журнала записи отладочной информации при краше системы:' -ForegroundColor Magenta
#$PC = 'wsir-it-05'
try {
    Get-WmiObject -Class Win32_OSRecoveryConfiguration -ComputerName $PC -EnableAllPrivileges -ErrorAction Stop -ErrorVariable $ErrDisDebugInfo | Set-WmiInstance -Arguments @{ DebugInfoType=0 }
    #Set-WmiInstance -Class Win32_OSRecoveryConfiguration -ComputerName $PC -Arguments @{ DebugInfoType=1 }
    Write-Host 'Запись отладочной информации отключена' -ForegroundColor Green
}
catch {
    $ErrDisDebugInfoMy = 'Отключить ведение журнала записи отладочной информации при краше системы не удалось'
    Write-Host $ErrDisDebugInfoMy -ForegroundColor Red
    Write-Host $ErrDisDebugInfo -ForegroundColor Red
    $Error[0]
    $ErrDisDebugInfoArray =  $ErrDisDebugInfoMy, $ErrDisDebugInfo, $Error[0]
    #$ErrDisDebugInfoArray
}
[Environment]::NewLine



### индексный файл службы поиска Windows ###
[Environment]::NewLine
#$PC = 'wsir-IT-05'
#$PC = 'VCSIR-ANGARA'
try{
    $FindIndexPath = "\\$PC\C$\ProgramData\Microsoft\Search\Data\Applications\Windows\Windows.edb"
    $FindIndexFile = Get-ChildItem $FindIndexPath -Force -recurse -ErrorAction Stop -ErrorVariable ErrGcifip
    
    $Size = [Math]::round($FindIndexFile.length / 1GB, 2)
    Write-Host "Размер Индексного файла службы поиска Windows в Гигабайтах:" -ForegroundColor Magenta
    Write-Host $Size -ForegroundColor Magenta
    [Environment]::NewLine
    
    #$PC = 'wsir-IT-05'
    try{
        $wsService = get-service -ComputerName $PC -Name WSearch -ErrorAction Stop -ErrorVariable ErrGsws
        $wsService
        $wsService.Stop()
        #(get-service -ComputerName $PC -Name WSearch).Stop()
        
        Start-Sleep -Seconds 10
        #$wsService #получит статус у сохранённой переменной, а не у фактически работающего сервиса
        get-service -ComputerName $PC -Name WSearch -ErrorAction Stop -ErrorVariable ErrGsws

        #Remove-Item -Path "C:\Test\hidden-RO-file.txt" -force -verbose -ErrorVariable ErrDelFindIndexFile
        Remove-Item $FindIndexFile -force -verbose -ErrorVariable ErrDelFindIndexFile #-confirm
        if ($ErrDelFindIndexFile) {
            $ErrDelFindIndexFile = "Пересоздать индексный файл службы поиска Windows не удалось"
            Write-host $ErrDelFindIndexFile
        }
        else{
            $wsService.Start()
            #(get-service -ComputerName $PC -Name WSearch).Start() -verbose
            Start-Sleep -Seconds 10
            #$wsService #получит статус у сохранённой переменной, а не у фактически работающего сервиса
            get-service -ComputerName $PC -Name WSearch -ErrorAction Stop -ErrorVariable ErrGsws
    
            Write-Host "Индексный файл службы поиска Windows пересоздан успешно" -ForegroundColor Green
        }
    }
    catch{
        $ErrGswsMy = 'Не удалось получить доступ к службе поиска Windows'
        Write-Host $ErrGswsMy -ForegroundColor Red
        Write-Host $ErrGsws -ForegroundColor Red
        $Error[0]
        $ErrGswsArray =  $ErrGswsMy, $ErrGsws, $Error[0]
    }
}
catch{
    $ErrGcifipMy = 'Недоступен путь до индексного файла службы поиска Windows'
    Write-Host $ErrGcifipMy -ForegroundColor Red
    Write-Host $ErrGcifip -ForegroundColor Red
    $Error[0]
    $ErrGcifipArray =  $ErrGcifipMy, $ErrGcifip, $Error[0] 
}
[Environment]::NewLine
#PAUSE
...
Write-Host 'Папки профилей, которые НЕ будут полностью удаляться:' -ForegroundColor Green
#$UserProp.SamAccountName = 'Polina.Mosichenko'
#Get-LocalUser
$NoDelUsers = 'Все пользователи','All Users','Default','Default User','Администратор','Administrator','Public','Общие','desktop.ini', 'Skype', 'Trueconf', 'TrueConf', $UserLogin #$UserProp.SamAccountName
$NoDelUsers
[Environment]::NewLine

Write-Host 'Удаление пользователей:' -ForegroundColor Magenta
...
#>
# Удаление папок/профилей неиспользуемых пользователей #2:
#$PC = 'WSZI-NGTIATE-03'
#$PC = 'wszi-IT-05'
try{
    $gwmioup = Get-WMIObject -class Win32_UserProfile -ComputerName $pc -ErrorAction Stop -ErrorVariable ErrGwmioUserProfile

    $wcusers = "\\$PC\C$\Users\"
    $UserPropsfol = Get-ChildItem $wcUsers -Force
    foreach ($UF in $UserPropsfol) {
      Write-Host $UF #выводим на экран логин пользователя
      if (($UF.lastwritetime -lt (Get-Date).AddDays(-60)) -and $NoDelUsers -notcontains $UF.name){
    
        try{
            Write-Host "Пробуем удалить профиль средствами Windows:" -ForegroundColor Magenta
            ...
            $gwmioup | Where-Object {$_.LocalPath -like 'C:\Users\'+$UF.name} | Remove-WmiObject -Verbose -ErrorVariable ErrDelUF1 # -AsJob
            <# ErrDelUF1 всегда не нуль, даже когда профиль удаляется корректно, поэтому отключаю
            if ($Null -ne $ErrDelUF1){
                Write-Host "Удалить профиль средствами Windows не удалось." -ForegroundColor Red
                $ErrDelUF1
                PAUSE
            }#>
            Write-Host "Профиль $UF удалён" -ForegroundColor Magenta
        }catch{
            Write-Host "Иногда средствами Windows удалить профиль не получается, поэтому пробуем чистить все вложенные папки профиля." -ForegroundColor Magenta
            Write-Host "Если на предыдущем этапе профиль пользователя удалился успешно, то далее получим ошибку доступа к его уже не существующей папке." -ForegroundColor Magenta
            Remove-Item -Path "\\$PC\C$\Users\$UF\" -force -recurse -verbose -ErrorVariable ErrDelUF2
    
            Write-Host "Профиль $UF очищен" -ForegroundColor Magenta
        }  
        [Environment]::NewLine
        #PAUSE
      }#if (($UF.lastwritetime -lt (Get-Date).AddDays(-60)) -and $NoDelUsers -notcontains $UF.name){    
    ...
    $ErrGwmioUserProfileMy = 'Не удалось получить профили пользователей Windows'
    #$ErrGwmioUserProfileMyArray =  $ErrGwmioUserProfileMy, $ErrGwmioUserProfile, $Error[0]
    #Write-Host $ErrGwmioUserProfileMyArray -ForegroundColor Red
    Write-Host $ErrGwmioUserProfileMy -ForegroundColor Red
    Write-Host $ErrGwmioUserProfile -ForegroundColor Red
    $Error[0]
    $ErrGwmioUserProfileMyArray =  $ErrGwmioUserProfileMy, $ErrGwmioUserProfile, $Error[0]  
}
[Environment]::NewLine
...
Write-Host 'Очистка различных папок в системе:' -ForegroundColor Magenta
...
<#Это так не работает. Отключаю
if ($ErrDelWinOld) {
    $ErrDelWinOld = "Папка Windows.Old не смогла удалиться полностью. Можешь подключиться к диску C: и удалить её вручную:", $wc
    #$ErrDelWinOld
}#>
...
foreach ($DFS in $DownFoldFiles) {  
  ...
  if ($DFS.lastwritetime -lt $Add10Days){
    ...
    #Remove-Item -Path $FilePath -force -verbose -confirm
     Remove-Item $DFS -force -verbose -recurse #-confirm
...
Write-Host "Очистка Корзины" -ForegroundColor Magenta
# $PC = 'WSIR-IT-01'
...
start-job -scriptblock {
    $wshell = New-Object -ComObject Wscript.Shell
    $wshell.Popup("Запущено сканирование диска C: на поиск 20 самых больших файлов. Это может занять продолжительное время в зависимости от размера диска. По некоторым директориям будут возникать ошибки про отсутствие доступа - ничего страшного. Это последний этап очистки. Если данная активность не интересна, можешь прервать скрипт.")
}
...
Write-Host "!!!" -ForegroundColor Red
Write-Host "!!!" -ForegroundColor Red
Write-Host "Поиск 20 самых больших файлов на диске С: завершён" -ForegroundColor Red
Write-Host "Далее по каждому из них будет предложено удалить его вручную" -ForegroundColor Red
Write-Host "Будь крайне внимателен и не удаляй то, в чём на 100% не уверен!!!" -ForegroundColor Red
Write-Host "!!!" -ForegroundColor Red
Write-Host "!!!" -ForegroundColor Red
Write-Host "!!!" -ForegroundColor Red

$wshell = New-Object -ComObject Wscript.Shell
$wshell.Popup("Ты ознакомился с красным текстом в командной строке скрипта удалённой очистки про ручное удаление самых больших файлов на диске С?")

$wshell = New-Object -ComObject Wscript.Shell
$wshell.Popup("Предупреждаю ещё раз: НЕ УДАЛЯЙ ТОГО, В ЧЁМ НА 100% НЕ УВЕРЕН!!!")

[Environment]::NewLine

foreach ($BFC in $BiggestFilesOnC) {  
  #Write-Host $BFC.directory -ForegroundColor Red
  #Write-Host $BFC.name -ForegroundColor Red
  #Write-Host "$BFC.PSPath" -ForegroundColor Red
  #[Environment]::NewLine

  $FilePath = -join($BFC.directory , "\" , $BFC.name)
  Write-Host $FilePath -ForegroundColor Red
  ...
  Remove-Item -Path $FilePath -force -verbose -confirm #-ErrorVariable E
  [Environment]::NewLine
} #
...
#$PC = 'wsir-it-05'
try{
    Get-WmiObject -Class Win32_diskdrive -ComputerName $PC -ErrorAction Stop -ErrorVariable ErrGwodd | Format-Table -Property Model, @{Label="Size(GB)"; Expression={$_.Size/1Gb}} -AutoSize
    [Environment]::NewLine
    ...
    try{
        ...
        $pi = Get-WmiObject -Class Win32_LogicalDisk -ComputerName $PC -ErrorAction Stop -ErrorVariable ErrGwold | Where-Object {$_.deviceid -like 'C:'}

        Write-host 'Стало :' -ForegroundColor Magenta
        "Disk " + $pi.DeviceID + " FreeSpace : {0}" -f ($pi.FreeSpace / 1GB).ToString("F01") + ' GB'
    }catch{
        $ErrGwoldMy = 'Не удалось загрузить информацию о диске С:'
        Write-Host $ErrGwoldMy -ForegroundColor Red
        Write-Host $ErrGwold -ForegroundColor Red
        $Error[0]
        #Write-Host $Error[0] -ForegroundColor Red
        #$ErrGwoddArray =  $ErrGwoddMy, $ErrGwodd, $Error[0] 
    }    
}catch{
    $ErrGwoddMy = 'Не удалось загрузить информацию о дисках'
    Write-Host $ErrGwoddMy -ForegroundColor Red
    Write-Host $ErrGwodd -ForegroundColor Red
    $Error[0]
    #Write-Host $Error[0] -ForegroundColor Red
    #$ErrGwoddArray =  $ErrGwoddMy, $ErrGwodd, $Error[0] 
}
...
$ErrGwmioUserProfileMyArray
[Environment]::NewLine
#Ошибки службы поиска Windows:
$ErrGcifipArray
$ErrGswsArray
$ErrDelFindIndexFile
[Environment]::NewLine
#$ErrDelWinOld #не понятно, как это фиксировать. отключаю
#[Environment]::NewLine



# Завершение Логирования в БД НОВОЕ
<#
$FeedBack = Read-Host "Напиши в свободной форме, как улучшить скрипт "
if ($FeedBack.Equals("") -or $FeedBack.Equals("+")) {
$FeedBack = $Null
#"null"
}   #else { $FeedBack  }
#>
# Фиксируем в таблицу ScriptExecute
ScriptsExecute -Operation Update -AdminLogin $AdminLogin -DateStart $DateStart -DateEnd $DateEnd -ScriptName $ScriptName -MyError $PCinfoResult.ErrGetServEngGroup #-Feedback $FeedBack
[Environment]::NewLine
