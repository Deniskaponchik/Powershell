...
IT_INFRA_SERVICE_SUPPORT@service.tele2.ru
с темой письма:
...
function InfoPcUser {
# $PCname = 'OSHIMKOVICH'   $PCname = 'wsir-it-02'
try{''
  ...
  #>   
   ...
   if ($ErrGWI) {
     ...
     # 'http://spareparts/site/koordinator/checkpc_n.php#'
     if (!$ie_procid) {
     $Global:ie_procinfo = Start-Process chrome -ArgumentList 'http://spareparts/site/koordinator/checkpc_n.php' #-wait #-passthru
     ...
     #(Get-ADComputer -Identity $PCname).enabled
...
#Import-CSV -Path "D:\Scripts\Test_branch_comp.csv" -Delimiter ";" -Encoding Default | ForEach-Object {
  ($user, $owner, $Rename, $Move, $Error[0], $IpCut, $NewParam, $Result1, $Result2, $ip, $UserOU, $Branch, $Excel_procid, $Excel_procinfo) = $null
  ''
      ...
      Import-CSV -Path "\\t2ru\folders\IT-Support\Региональные сети\CutPCnameSubnetBranch.csv" -Delimiter ";" -Encoding Default | ForEach-Object {
        if (($Move, $Rename) -eq 1) {}#'Пустой прогон оставшихся подсетей'}
           ...
           $ConBra = Select-String -InputObject $SplitBranch -Pattern $Branch   # Ищем бранч в двойных брнанчах
            ...
            #} else {'Не успех'}
             ...
             # Write-Host $Subnet1, $Subnet2, $Subnet3, $Subnet4, $Subnet5 -ForeGroundColor Yellow
                ...
                if ($City -like $Branch -or ($ConBra -and !$ConName)) {#'Успех'}
                    ...
                    if($error[0] -notlike '*найти объект*'){
                      ...
                      do {''
                         ...
                         #Rename-Computer -NewName WSIR-IT-03 -ComputerName WSIR-IT-01 -DomainCredential $AdmWSDomCred -restart -confirm -ErrorVariable ErrRen
                        $PingNew = (Test-Connection -ComputerName $NewPCName -Quiet -Count 15)
                        if ($PingNew) {Write-host 'Новое имя уже пингуется!' -ForeGroundColor Magenta}
                      } while ($x -ne 0 -and $ErrRen)
              ...
              ''
               ...
               $ResultMove = MovePC
               $Move = $ResultMove[2]
               $NewParam = $ResultMove[1]
               $Result1 = $ResultMove[0]
               #>
               # Pause
               <#
               if ($branch -like 'CBIT AND REGION'){$Branch = Read-Host 'Укажи Бранч:'}
               #$Branch = 'CC-Irkutsk'
               $TargetOU = "OU=Computers,OU=$branch,OU=Branches,DC=corp,DC=tele2,DC=ru"
               ''
               'Учётка компьютера должна находиться в этом бранче:'
               $branch
               $TargetOU
               Get-ADComputer -Identity $PCname | Move-ADObject -TargetPath $TargetOU -verbose -confirm
               $Move = 1
               $NewParam = $Branch
               $Result1 = 'Перенесён'
               ''
            ...
            Write-Host '\\t2ru\folders\IT-Support\Региональные сети\CutPCnameSubnetBranch.csv' -ForeGroundColor Yellow
            #Start-Process -FilePath '\\t2ru\folders\IT-Support\Региональные сети\CutPCnameSubnetBranch.csv'
            if (!$Excel_procid) {$Excel_procinfo = Start-Process Start-Process -FilePath '\\t2ru\folders\IT-Support\Региональные сети\CutPCnameSubnetBranch.csv'
 ...
 $res += "$PCname`t$City`t$IpCut`t$Result1`t$NewParam`t$Result2"  # Добавляем строку в итоговый файл
}
