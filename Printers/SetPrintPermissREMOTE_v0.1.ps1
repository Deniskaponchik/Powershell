...
#Get-Printer | ForEach-Object {Add-PrinterPermission -$User Service.Ast -$AceType Allow -$AccessMask ManagePrinters}
#Get-Printer | Add-PrinterPermission -$User Service.Ast -$AceType Allow -$AccessMask ManagePrinters
...
Get-Printer | ForEach ($Printer in $Printers) {
     Add-PrinterPermission -$user Service.Ast -$AceType Allow -$AccessMask 
}
<#
Get-Printer | ForEach-Object {Add-PrinterPermission -User Service.Ast -AceType <name> -AccessMask <name>}
