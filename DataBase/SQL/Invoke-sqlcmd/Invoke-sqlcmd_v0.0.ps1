...
while ($StartDateTime -le $EndDateTime) {
 ...
 #$StartDate = $StartDateTime.ToString("yyyy-MM-dd")  # не работает
  ...
  #$InvokeVariables = @("StartDate000000 = '$StartDate000000'", "StartDate235959 = '$StartDate235959'")
   ...
   Invoke-sqlcmd -ServerInstance $dataSource -Database $database -InputFile $FileSQL -Variable $InvokeVariables | Export-Csv -Path "$PathScript\$NameScript\$startDate.csv" -Encoding UTF8
   #-Credential -OutputAs
