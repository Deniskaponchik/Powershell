...
[Environment]::NewLine
 ...
 else {[DateTime]$StartDateTime = $StartDateTime}
 ...
 else {[DateTime]$EndDateTime= $EndDateTime}
...
$CsvFolder = "D:\Scripts\PowerShell\SQL\TFS_370389_v2.0"

$Query = @"
$FileSqlTxt
"@
$dataSource = "t2ru-tr-tst-02"                # t2ru-bpmdb-read\bpmonline t2ru-tr-tst-02
$database = "BPMonline7_8888"
$auth = "Integrated Security=SSPI;"
[DateTime]$StartDateTime = "2021-07-25"       # 2022-01-01 2021-07-25  
[DateTime]$EndDateTime = "2021-07-27"         # Get-Date   "2021-07-27"
#>
...
while ($StartDateTime -le $EndDateTime) {
   ...
   #$StartDate = $StartDateTime.ToString("yyyy-MM-dd")  # не работает
    ...
    $DateForSqlVariable
    $DateForCsvName = '{0:ddMMyyy}' -f $StartDateTime
    ...
    $Table | Export-Csv -Path "$CsvFolder\SR_BPM7_$DateForCsvName.csv" -Encoding UTF8 -NoTypeInformation
