--Объявлять переменные не нужно (они задаются Парметрами в SQLconnect)
--DECLARE @StartDate varchar(255) = $(StartDate)

select
c.Subject [Описание]
--replace(c.Subject, char(10), ' ') [Описание]
...
--cast (dateadd (hh, +3, cl.StartDate) as date) = @StartDate
--select cast (dateadd(day,-1,GETDATE()) as date)
--cl.StartDate >= dateadd(day,-1,getdate())
--CONVERT(date,cl.StartDate,101) = '2021-07-25'
  CONVERT(date,cl.StartDate,101) = @StartDate
